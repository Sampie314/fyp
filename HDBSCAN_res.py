...
from handlers.HDBSCANHandler import HDBSCANHandler as ch
...
best_chromosome = {'num_heads': 2, 'feed_forward_dim': 64, 'num_transformer_blocks': 1, 
                   'mlp_units': 128, 'dropout_rate': 0.1, 'learning_rate': 0.001, 'num_mlp_layers': 3, 
                   'num_epochs': 25, 'activation_function': 1, 'batch_size': 2048}
...
close_preds.to_pickle('predictions/HDBSCAN_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/HDBSCAN_log_returns_preds.pkl')