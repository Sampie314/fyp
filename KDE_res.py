...
best_chromosome = {'num_heads': 1, 'feed_forward_dim': 128, 'num_transformer_blocks': 3, 
                   'mlp_units': 128, 'dropout_rate': 0.01, 'learning_rate': 5e-06, 
                   'num_mlp_layers': 3, 'num_epochs': 200, 'activation_function': 0, 
                   'batch_size': 2048}
...
close_preds.to_pickle('predictions/kde_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/kde_log_returns_preds.pkl')
