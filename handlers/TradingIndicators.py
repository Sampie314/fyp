...
def calculate_aroon_oscillator(df, period=25) -> pd.DataFrame:
    ...
    df['Periods_Since_High_Close'] = df['Close'].rolling(window=period, min_periods=1).apply(lambda x: period - x.argmax() - 1)
    
    # Calculate the number of periods since the lowest close over the specified period
    df['Periods_Since_Low_Close'] = df['Close'].rolling(window=period, min_periods=1).apply(lambda x: period - x.argmin() - 1)
