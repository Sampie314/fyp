...
best_chromosome = {'num_heads': 4, 'feed_forward_dim': 128, 'num_transformer_blocks': 2, 
                   'mlp_units': 256, 'dropout_rate': 0.1, 'learning_rate': 5e-06, 'num_mlp_layers': 5, 
                   'num_epochs': 200, 'activation_function': 0, 'batch_size': 1024}
...
close_preds.to_pickle('predictions/GMM_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/GMM_log_returns_preds.pkl')