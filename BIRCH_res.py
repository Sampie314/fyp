...
from handlers.BIRCHHandler import BIRCHHandler as ch 
...
best_chromosome = {'num_heads': 2, 'feed_forward_dim': 128, 'num_transformer_blocks': 2, 
                   'mlp_units': 256, 'dropout_rate': 0.2, 'learning_rate': 1e-05, 
                   'num_mlp_layers': 8, 'num_epochs': 500, 'activation_function': 1, 'batch_size': 512}
...
close_preds.to_pickle('predictions/BIRCH_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/BIRCH_log_returns_preds.pkl')