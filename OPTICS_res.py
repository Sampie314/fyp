...
best_chromosome = {'num_heads': 1, 'feed_forward_dim': 64, 'num_transformer_blocks': 3, 
                   'mlp_units': 256, 'dropout_rate': 0.01, 'learning_rate': 1e-05, 
                   'num_mlp_layers': 8, 'num_epochs': 50, 'activation_function': 1, 'batch_size': 512}
...
close_preds.to_pickle('predictions/OPTICS_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/OPTICS_log_returns_preds.pkl')
