...
from handlers.WardHierarchicalHandler import WardHierarchicalHandler as ch
...
best_chromosome = {'num_heads': 3, 'feed_forward_dim': 64, 'num_transformer_blocks': 3, 
                   'mlp_units': 256, 'dropout_rate': 0.01, 'learning_rate': 1e-05, 
                   'num_mlp_layers': 5, 'num_epochs': 25, 'activation_function': 1, 'batch_size': 256}
...
cls = ch(train_X.drop(columns=stationary_cols + ['symbol']), train_y.drop(columns=['symbol']), 
                     ...
                     n_clusters=None, max_clusters=10, mergeCluster=True, plt=False, pltNo=0)
...
close_preds.to_pickle('predictions/ward_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/ward_log_returns_preds.pkl')
