...
best_chromosome = {'num_heads': 3, 'feed_forward_dim': 128, 'num_transformer_blocks': 2, 
                   'mlp_units': 64, 'dropout_rate': 0.2, 'learning_rate': 1e-05, 
                   'num_mlp_layers': 8, 'num_epochs': 100, 'activation_function': 1, 'batch_size': 128}
...
close_preds.to_pickle('predictions/Kmeans_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/KMeans_log_returns_preds.pkl')
