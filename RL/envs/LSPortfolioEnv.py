...
class LSPortfolioOptimizationEnv(gym.Env):
    """ Portfolio optimisation env for long short neutral portfolios """
    ...
    def __init__(
        ...
        self._sorted_times = sorted(set(self.df[time_column]))
            ...
            self.observation_space = spaces.Dict(
                {
                    ...
                    "last_action": spaces.Box(low=-1, high=1, shape=(action_space,)),
                }
            )
        else:
    ...
    def step(self, actions):
        ...
        if self._terminal:
            ...
            weights = self._normalize_long_short_weights(actions)
    ...
    def reset(self, seed: int = None):
        ...
        """
        # set seed as compliant with new gymnasium API
    ...
    def _get_state_and_info_from_time_index(self, time_index):
        ...
        self._data = self.df[
            (self.df[self._time_column] >= start_time)
            & (self.df[self._time_column] <= end_time)
    ...
    def _normalize_long_short_weights(self, actions):
        """Normalizes the action vector for a long-short portfolio.
        Ensures the sum of weights is 0 and the sum of absolute weights is 1."""
        # Ensure sum of weights is 0
    ...
    def _preprocess_data(self, order, normalize, tics_in_portfolio):
        ...
        if order:
            self.df = self.df.sort_values(by=[self._tic_column, self._time_column])
        ...
        self.df[self._features] = self.df[self._features].astype("float32")
    ...
    def _normalize_dataframe(self, normalize):
        ...
        if type(normalize) == str:
            if normalize == "by_fist_time_window_value":
                ...
                self.df = self._temporal_variation_df(self._time_window - 1)
    ...
    def save_action_memory(self):
        ...
        df_actions.columns = list(self._tic_list)
