...
from handlers.DBSCANHandler import DBSCANHandler as ch
...
best_chromosome = {'num_heads': 2, 'feed_forward_dim': 128, 'num_transformer_blocks': 3, 
                   'mlp_units': 64, 'dropout_rate': 0.05, 'learning_rate': 0.0001, 
                   'num_mlp_layers': 3, 'num_epochs': 50, 'activation_function': 0, 'batch_size': 1024}
...
close_preds.to_pickle('predictions/DBSCAN_close_preds.pkl')

log_returns_preds = pd.DataFrame(log_returns_preds, index=test_X.index)
log_returns_preds['symbol'] = test_X['symbol']
log_returns_preds.to_pickle('predictions/DBSCAN_log_returns_preds.pkl')